...
Component({
  ...
  methods: {
    ...
    * @Bug：
        ...
        但现在错误情况是：父向子的属性properties传递数据》在子组件属性methods中的自定义事件方法（获得并处理子组件属性properties中的数据，然后将处理结果数据赋值给了子组件属性data: {tabsArr3:tabsArr3Var}，即子未向父传递数据）》在父组件属性methods中的自定义事件方法（获得并处理子组件传递来的数据）。
    */
