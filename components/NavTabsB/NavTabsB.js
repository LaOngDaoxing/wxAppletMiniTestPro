...
Component({
   ...
   * @Remark
      1. 父组件通过子组件的属性properties，给子组件传递参数 
      2. 子组件通过父组件的事件methods，给父组件传递参数 
   *
   */
  properties: {
  ...
  methods: {
    ...
    * @Bug：
        正确的逻辑是：父向子传递数据》在子组件的自定义事件方法中（获得并处理父组件传递来的数据 ——> 子向父传递数据）》在父组件的自定义事件方法中（获得并处理子组件传递来的数据）。
        但现在错误情况是：父向子传递数据》在子组件的自定义事件方法中（获得并处理父组件传递来的数据，然后将处理结果数据赋值给了子组件data: {tabsArr3:tabsArr3Var}，即子未向父传递数据）》在父组件的自定义事件方法中（获得并处理子组件传递来的数据）。
    */
